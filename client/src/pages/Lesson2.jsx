...
export default function Lesson2() {
  ...
  // Idle detection - show suggestion after 15 seconds of inactivity
  useEffect(() => {
    if (gameState === 'conversation' && !isLoading) {
  ...
  }, [gameState, isLoading, conversationTurn]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Get AI response
      const level = getLevelData();
      const persona = `${level.scenario.character} - ${level.scenario.situation}. Emotion: ${level.scenario.emotion}. Background: ${level.scenario.background}`;

      const data = await simulate({
        message: currentMessage,
        persona: persona,
        sessionId: sessionId // Use tracked sessionId
      });
      ...
      setTranscript([
        ...newTranscript,
        { speaker: level.scenario.character, text: data.reply || data.message || 'No response' }
      ]);
  ...
  const handleEvaluate = async () => {
    if (!sessionId) {
      setEvaluation({
        ...
        negotiation: 0,
        averageScore: 0,
        overall_feedback: 'No conversation to evaluate. Start a conversation first.',
        suggestions: ['Begin by greeting the borrower', 'Establish rapport before discussing payment']
      });
    ...
    try {
      ...
      // Call REAL backend evaluation API
      const data = await evaluate(sessionId);
      ...
      // Reload progress
      const progress = getLessonProgress(2);
      setLessonProgress(progress);
      ...
      setEvaluation({
        ...
        negotiation: 0,
        averageScore: 0,
        overall_feedback: `Unable to get evaluation: ${error.message}. Please check your connection.`,
        suggestions: ['Ensure the server is running', 'Check your internet connection', 'Try the conversation again']
      });
      setGameState('outcome');
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950">
      <Navbar />
      <LessonProgressSidebar currentLessonId={2} />
      ...
      <div className="max-w-5xl mx-auto px-4 pt-24 pb-20 pr-[420px]">
        ...
        <div className="mb-8 flex items-center gap-2">
          {ADVENTURE_LEVELS.map((level) => {
            ...
            const isLocked = !isLevelUnlocked(2, level.level);
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-slate-300 space-y-1 text-sm">
                          {conversationTurn === 1 && (
                            <>
                              <p>• "Hi, I understand this can be difficult..."</p>
                              <p>• "I'm calling about your account today..."</p>
                              <p>• "I appreciate you taking my call..."</p>
                            </>
                          )}
                          {conversationTurn === 2 && (
                            <>
                              <p>• Show empathy for their situation</p>
                              <p>• Ask about what led to the missed payments</p>
                              <p>• Acknowledge their concerns professionally</p>
                            </>
                          )}
                          {conversationTurn >= 3 && (
                            <>
                              <p>• Offer payment plan options</p>
                              <p>• Ask "What payment amount works for you?"</p>
                              <p>• Summarize next steps clearly</p>
                            </>
                          )}
                        </div>
                      </div>
